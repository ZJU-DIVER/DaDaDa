...
if __name__ == '__main__':
    ...
    for category in categories:
        ...
        cur_X = X[X[category] == 1]
        cur_X = cur_X.drop(columns=category_list)
        cur_y = y[X[category] == 1]
        ...
        for model_name, model in models.items():
            ...
            for train_index, test_index in kf.split(cur_X):
                ...
                if mode == 'train':
                    ...
                    # store the model
                    model_filename = f'{cur_save_path}/{model_name}_fold{fold}.pkl'
                    with open(model_filename, 'wb') as f:
                        pickle.dump(model, f)
                else:
                    model = pickle.load(open(f'{cur_save_path}/{model_name}_fold{fold}.pkl', 'rb'))
                    y_pred = model.predict(X_test)
                    # compute scores
                    r2_scores.append(r2_score(y_test, y_pred))
                    mae_scores.append(mean_absolute_error(y_test, y_pred))
