...
if __name__ == '__main__':
    ...
    mode = args.mode
    file_path, save_path = get_file_path_and_save_path(args.file)
    ...
    categories = ["category_Financial Services Data", "category_Retail, Location and Marketing Data"]
    for category in categories:
        cur_save_path = f'{save_path}/{category}'
        results = {}
    
        cur_df = df[df[category] == 1]
        cur_df = cur_df.drop(columns=category_list)
        target = 'price_log'
        features = [col for col in cur_df.columns if col != target]
        X = cur_df[features].values
        y = cur_df[target].values
        # create dataset
        ...
        if mode == 'train':
            ...
            # save the model
            torch.save(model.state_dict(), f"{cur_save_path}/dnn_model_{num_epochs}.pth")
        else:
            # load the model
            model = DNNModel(input_dim=X.shape[1])
            num_epochs = 500
            model.load_state_dict(torch.load(f"{cur_save_path}/dnn_model_{num_epochs}.pth"))
            model.eval()
            criterion = nn.MSELoss()
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            final_metrics = validate(model, test_loader, criterion, device)
